...
public:
    ...
    struct WriteAccess {
        ...
        WriteAccess(const K& key, Segment& segment)
                : guard(segment._mutex), ref_to_value(segment._data[key]) {
        }
    };
    ...
    explicit ConcurrentMap(size_t bucket_count) : _segments(bucket_count)
    {
    }

    WriteAccess operator[](const K& key) {
        Segment& segment = _segments[hasher(key)];
        return { key, segment };
    }
    ...
    MapType BuildOrdinaryMap() const {
        unordered_map<K, V, Hash> whole_map;
        for (auto& segment : _segments) {
            lock_guard<mutex> guard(segment._mutex);
            whole_map.insert(segment._data.begin(), segment._data.end());
        }
        return whole_map;
    }
